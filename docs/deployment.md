...
默认预览地址为 `http://127.0.0.1:8000/`；修改 `docs/` 后自动刷新。
修改外部源文件时重新触发构建，或使用 `mkdocs serve --watch crb_ros_msg --watch casbot2_cpp_demo --watch casbot2_py_demo --watch casbot2_cpp_tests --watch casbot2_py_tests`。
...
| 属性 | 值 |
| --- | --- |
| `.readthedocs.yaml` | RTD 的操作系统、Python 3.11、依赖安装和 MkDocs 构建配置 |
| `requirements.txt` | 文档依赖，锁定已验证的 MkDocs 1.x 与主题／插件版本 |
| `mkdocs.yml` | 导航、主题、中文搜索、Mermaid、语言 Tab 与生成插件 |
...
2. 选择实际使用的项目名，默认分支设为 `main`，配置文件为根目录 `.readthedocs.yaml`。
...
新增接口字段和开发指南所引用的源代码也会改变生成文档，不能只监听 `docs/`。
...
`.github/workflows/docs.yml` 在相关 PR、main 更新及手动触发时安装 `requirements.txt`、执行严格构建并保存 HTML artifact。
这提供独立于 RTD 的构建检查，不会执行机器人控制程序，也不负责创建 RTD 托管项目。
