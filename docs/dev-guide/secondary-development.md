...
|---|---|---|---|---|---|---|
|1|leg\_l1\_joint<br>|\[\-1\.91986，1\.5708\]<br>|leg\_r1\_joint|\[\-1\.91986，1\.5708\]<br>|髋部旋转||
|2|leg\_l2\_joint|\[\-0\.17453，1\.5708\]|leg\_r2\_joint|\[\-1\.5708，0\.17453\]|髋部滚转||
|3|leg\_l3\_joint|\[\-1\.5708，1\.5708\]<br>|leg\_r3\_joint|\[\-1\.5708，1\.5708\]<br>|髋部偏航<br>||
|4|leg\_l4\_joint|\[0，2\.53073\]<br>|leg\_r4\_joint|\[0，2\.53073\]<br>|膝盖俯仰||
|5|leg\_l5\_joint|\[\-0\.87266，0\.50614\]<br>|leg\_r5\_joint|\[\-0\.87266，0\.50614\]<br>|脚踝俯仰<br>||
|6|leg\_l6\_joint|\[\-0\.50614，0\.50614\]|leg\_r6\_joint|\[\-0\.50614，0\.50614\]|脚踝滚转||
...
|---|---|---|---|---|---|---|---|---|---|
|**顺序1**|Base\_向X方向平移<br>（机器人左右横移）<br>|leg\_l1\_joint<br>|leg\_r1\_joint|left\_shoulder\_pitch\_joint|right\_shoulder\_pitch\_joint|left\_thumb\_metacarpal\_joint<br>|right\_thumb\_metacarpal\_joint|head\_yaw\_joint|waist\_yaw\_joint|
|**顺序2**|Base\_向Y方向平移\(机器人面部朝向\-前后运动）|leg\_l2\_joint|leg\_r2\_joint|left\_shoulder\_roll\_joint|right\_shoulder\_roll\_joint|left\_thumb\_proximal\_joint|right\_thumb\_proximal\_joint|head\_pitch\_joint||
|**顺序3**|Base\_向Z方向平移<br>（机器人上下运动）|leg\_l3\_joint|leg\_r3\_joint|left\_shoulder\_yaw\_joint|right\_shoulder\_yaw\_joint|left\_index\_proximal\_joint|right\_index\_proximal\_joint|||
|**顺序4**|Base\_Pitch方向旋转<br>（机器人前趴后仰）|leg\_l4\_joint|leg\_r4\_joint|left\_elbow\_pitch\_joint|right\_elbow\_pitch\_joint|left\_index\_distal\_joint（被动）|right\_index\_distal\_joint（被动）|||
|**顺序5**|Base\_Yaw方向旋转（机器人左右转身）|leg\_l5\_joint|leg\_r5\_joint|left\_wrist\_yaw\_joint|right\_wrist\_yaw\_joint|left\_middle\_proximal\_joint|right\_middle\_proximal\_joint|||
|**顺序6**|Base\_Roll方向旋转<br>（机器人左倾右倾）|leg\_l6\_joint|leg\_r6\_joint|left\_wrist\_pitch\_joint|right\_wrist\_pitch\_joint|left\_middle\_distal\_joint（被动）|right\_middle\_distal\_joint（被动）|||
...
[头显相关参考](https://blog.csdn.net/weixin_35987118/article/details/151871708)



## 运控开发

本节运控接口已按 main `c3b2902` 校正；完整模式守卫、增益布局和限制见[运动控制](motion-control.md)。
周边硬件、部署路径和训练环境仍为交付手册资料，需按实际交付版本核查。
...
`linear.y`：当前导航实现不使用此字段，不支持据此请求横移

`angular.z`：前后运动时的转向请求；当前导航实现不支持仅凭该字段原地旋转
...
|左手|left\_thumb\_metacarpal\_joint、left\_thumb\_proximal\_joint、left\_index\_proximal\_joint、left\_middle\_proximal\_joint、left\_ring\_proximal\_joint、left\_pinky\_proximal\_joint|
|右手|right\_thumb\_metacarpal\_joint、right\_thumb\_proximal\_joint、right\_index\_proximal\_joint、right\_middle\_proximal\_joint、right\_ring\_proximal\_joint、right\_pinky\_proximal\_joint|
|左腿|leg\_l1\_joint、leg\_l2\_joint、leg\_l3\_joint、leg\_l4\_joint、leg\_l5\_joint、leg\_l6\_joint|
|右腿|leg\_r1\_joint、leg\_r2\_joint、leg\_r3\_joint、leg\_r4\_joint、leg\_r5\_joint、leg\_r6\_joint|
...
这些是内部模式值：阻尼 1、准备 2、全身调试 7、上身调试 8。`/motion/current_mode` 仅在全身调试更新路径发布；`/get_robot_mode` 使用 ZERO/STAND/WALK 兼容映射。日常读取 `/motion/status` 区分真实模式。
...
topic name: /motion/debug/joint\_cmd

topic type: crb\_ros\_msg::msg::JointStateData

标准 `/motion/joint_cmd` 与 `/motion/joint_state` 则使用 `sensor_msgs/msg/JointState`，不能混用。

反馈话题：/motion/debug/joint\_state（同为 JointStateData，含当前生效 kp/kd）
...
casbot02强化学习环境基于Isaac Gym开发，采用AMP强化学习算法进行训练。框架与访问方式由交付团队提供，见[仿真环境获取](../getting-started/sim.md)。

**3\.4\.2 强化学习环境训练**

casbot02强化学习环境训练包括train、play、sim2sim，框架与访问方式由交付团队提供，见[仿真环境获取](../getting-started/sim.md)。
