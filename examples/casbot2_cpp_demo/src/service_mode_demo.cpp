...
bool wait_service(const std::shared_ptr<rclcpp::Client<ServiceT>> &client, const std::string &name, rclcpp::Node &node)
{
...
int main(int argc, char * argv[])
{
...

  {
    ...
    if (rclcpp::spin_until_future_complete(node, fut, 3s) == rclcpp::FutureReturnCode::SUCCESS) {
      if (!fut.get()->success) { rclcpp::shutdown(); return 1; }
    } else {
  ...
  if (!call_set_bool("/motion/switch_nav_mode", switch_nav, true)) { rclcpp::shutdown(); return 1; }
  if (!call_set_bool("/motion/switch_nav_mode", switch_nav, false)) { rclcpp::shutdown(); return 1; }
  if (!call_set_bool("/switch_teleoperation", switch_teleop, true)) { rclcpp::shutdown(); return 1; }
  if (!call_set_bool("/switch_teleoperation", switch_teleop, false)) { rclcpp::shutdown(); return 1; }
  if (!call_set_bool("/switch_autonomous", switch_auto, true)) { rclcpp::shutdown(); return 1; }
  if (!call_set_bool("/switch_autonomous", switch_auto, false)) { rclcpp::shutdown(); return 1; }
