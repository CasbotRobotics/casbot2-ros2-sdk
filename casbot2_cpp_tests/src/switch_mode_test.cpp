...
int main(int argc, char *argv[])
{
    ...
    auto node = rclcpp::Node::make_shared("switch_mode_test");

    RCLCPP_INFO(node->get_logger(), "[T02] === 切换遥操作模式 ===");
    if (!callSetBool(node, "/switch_teleoperation", true,  "switch_teleoperation")) { rclcpp::shutdown(); return 1; }
    rclcpp::sleep_for(std::chrono::seconds(2));
    if (!callSetBool(node, "/switch_teleoperation", false, "switch_teleoperation")) { rclcpp::shutdown(); return 1; }
    rclcpp::sleep_for(std::chrono::seconds(1));

    RCLCPP_INFO(node->get_logger(), "[T02] === 切换自主（动作播放）模式 ===");
    if (!callSetBool(node, "/switch_autonomous", true,  "switch_autonomous")) { rclcpp::shutdown(); return 1; }
    rclcpp::sleep_for(std::chrono::seconds(2));
    if (!callSetBool(node, "/switch_autonomous", false, "switch_autonomous")) { rclcpp::shutdown(); return 1; }
